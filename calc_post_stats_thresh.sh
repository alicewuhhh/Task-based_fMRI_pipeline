...
# Updated to include z-map thresholding and command-line subjects, Mar 2025
...
preprocess_subject() {
    ...
    flirt -in ${ROI}/STG_Heschl.nii.gz -ref ${SUBDIR}/fsl_stats/sub-${subject}_task-motor_run-01_contrasts.feat/stats/zstat1.nii.gz -applyxfm -usesqform -out ${ROI}/STG_Heschl_sub.nii.gz
    flirt -in ${ROI}/SMA_PMC.nii.gz -ref ${SUBDIR}/fsl_stats/sub-${subject}_task-motor_run-01_contrasts.feat/stats/zstat1.nii.gz -applyxfm -usesqform -out ${ROI}/SMA_PMC_sub.nii.gz
...
for subject in "$@"; do
    ...
    for task in motor_run-01 motor_run-02 lang; do
